...
void* vdev_create(int type, void *surface, int bufnum, int w, int h, int ftime, CMNVARS *cmnvars)
{
    ...
    c->tickframe   = ftime;  //ftime ��һ֡��ʱ����ms 1000 / fps
    c->ticksleep   = ftime;
    c->cmnvars     = cmnvars;
    return c;
}
...
void vdev_avsync_and_complete(void *ctxt)
{
    ...
    if (!(c->status & VDEV_PAUSE)) {
        ...
        tickframe   = 100 * c->tickframe / c->speed; //c->speed Ĭ�� 100
        tickcur     = av_gettime_relative() / 1000; //��ǰϵͳʱ��
        tickdiff    = (int)(tickcur - c->ticklast); //2֡��Ⱦ�ģ�ʵ���ϵģ�ʱ����
        c->ticklast = tickcur;

        //(tickcur - c->cmnvars->start_tick) �����˶�ã�ϵͳʱ��ʱ�䣬��λ���� ms
