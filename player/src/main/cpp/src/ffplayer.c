...
static void vfilter_graph_init(PLAYER *player) {
    ...
    strcat(fstr,
           player->init_params.video_deinterlace && player->init_params.video_rotate ? "[a];[a]"
                                                                                     : "");
    ...
    outputs->pad_idx = 0;
    outputs->next = NULL;
    ret = avfilter_graph_parse_ptr(player->vfilter_graph, player->init_params.filter_string[0]
                                                          ? player->init_params.filter_string
                                                          : fstr, &inputs, &outputs, NULL);
...
#define AVDEV_VFWCAP  "vfwcap"
    ...
    if (strstr(player->url, "rtsp://") == player->url
        ...
        if (player->init_params.avts_syncmode == AVSYNC_MODE_AUTO) {
            player->init_params.avts_syncmode =
                    memcmp(player->url, "rtmp://", 7) == 0 ? AVSYNC_MODE_LIVE_SYNC1
    ...
    while (1) {
        ...
        player->read_timeout = player->init_params.init_timeout ? player->init_params.init_timeout *
                                                                  1000 : -1;
    ...
    if (player->astream_index != -1) {
        ...
        if (player->acodec_context->channel_layout == 0) {
            player->acodec_context->channel_layout = av_get_default_channel_layout(
                    player->acodec_context->channels);
        }
        //-- fix audio channel layout issue
    }
    ...
    // open render
    player->render = render_open(player->init_params.adev_render_type,
                                 player->init_params.vdev_render_type,
                                 player->cmnvars.winmsg, player->vfrate,
                                 player->init_params.video_owidth,
    ...
    player->init_params.audio_channels = player->acodec_context ? av_get_channel_layout_nb_channels(
            player->acodec_context->channel_layout) : 0;
    player->init_params.audio_sample_rate = player->acodec_context
                                            ? player->acodec_context->sample_rate : 0;
...
static void *video_decode_thread_proc(void *param) {
    ...
    while (!(player->status & PS_CLOSE)) {
        ...
        while (packet->size > 0 && !(player->status & (PS_V_PAUSE | PS_CLOSE))) {
            ...
            av_log(NULL, AV_LOG_ERROR, "John elon debug %d==%d==%d==%d==%d==%d\n", player->vcodec_context->width,
                   player->init_params.video_vwidth, player->vcodec_context->height,
                   player->init_params.video_vheight,player->init_params.video_owidth,player->init_params.video_oheight);
...
void player_close(void *hplayer) {
    ...
    if (player->adecode_thread)
        pthread_join(player->adecode_thread, NULL); // wait audio decoding thread exit
    if (player->vdecode_thread)
        pthread_join(player->vdecode_thread, NULL); // wait video decoding thread exit
...
void player_getparam(void *hplayer, int id, void *param) {
    ...
    switch (id) {
        case PARAM_MEDIA_DURATION:
            *(int64_t *) param = player->avformat_context ? (player->avformat_context->duration *
                                                             1000 / AV_TIME_BASE) : 1;
            break;
...
void player_load_params(PLAYER_INIT_PARAMS *params, char *str) {
    char value[16];
    params->video_stream_cur = atoi(
            parse_params(str, "video_stream_cur", value, sizeof(value)) ? value : "0");
    params->video_thread_count = atoi(
            parse_params(str, "video_thread_count", value, sizeof(value)) ? value : "0");
    params->video_hwaccel = atoi(
            parse_params(str, "video_hwaccel", value, sizeof(value)) ? value : "0");
    params->video_deinterlace = atoi(
            parse_params(str, "video_deinterlace", value, sizeof(value)) ? value : "0");
    ...
    params->audio_stream_cur = atoi(
            parse_params(str, "audio_stream_cur", value, sizeof(value)) ? value : "0");
    params->audio_bufpktn = atoi(
            parse_params(str, "audio_bufpktn", value, sizeof(value)) ? value : "0");
    params->subtitle_stream_cur = atoi(
            parse_params(str, "subtitle_stream_cur", value, sizeof(value)) ? value : "0");
    params->vdev_render_type = atoi(
            parse_params(str, "vdev_render_type", value, sizeof(value)) ? value : "0");
    params->adev_render_type = atoi(
            parse_params(str, "adev_render_type", value, sizeof(value)) ? value : "0");
    ...
    params->waveout_device_id = atoi(
            parse_params(str, "waveout_device_id", value, sizeof(value)) ? value : "-1");
