...
#define AVDEV_VFWCAP  "vfwcap"
    ...
    player->render = render_open(player->init_params.adev_render_type,
                                 ...
                                 player->cmnvars.winmsg, vrate, player->init_params.video_owidth,
...
//          render_audio(player->render, &player->aframe);
        ...
        while (packet->size > 0 && !(player->status & (PS_A_PAUSE | PS_CLOSE))) {
            ...
            consumed = avcodec_decode_audio4(player->acodec_context, &player->aframe, &gotaudio,
                                             packet);
            if (consumed < 0) {
...
static void *video_decode_thread_proc(void *param) {
    ...
    while (!(player->status & PS_CLOSE)) {
        ...
        while (packet->size > 0 && !(player->status & (PS_V_PAUSE | PS_CLOSE))) {
            ...
            consumed = avcodec_decode_video2(player->vcodec_context, &player->vframe, &gotvideo,
                                             packet);
            if (consumed < 0) {
...
void player_load_params(PLAYER_INIT_PARAMS *params, char *str) {
    ...
    params->open_syncmode = atoi(
            parse_params(str, "open_syncmode", value, sizeof(value)) ? value : "0");
    ...
    params->waveout_device_id = atoi(
            parse_params(str, "waveout_device_id", value, sizeof(value)) ? value : "0");
